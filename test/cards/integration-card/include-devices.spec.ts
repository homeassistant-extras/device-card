    ...
    describe('shouldIncludeDevice', () => {
      ...
      it('should return true when device name matches an inclusion pattern', () => {
        ...
        expect(shouldIncludeDevice(config, 'device_id', 'Test Device')).to.be
          .true;
        expect(shouldIncludeDevice(config, 'id_123', 'Testing Unit')).to.be
          .true;
      });

      it('should return true when device name case sensitive matches an inclusion pattern', () => {
        // Arrange
        const config = {
          include_devices: ['*test*'],
        } as Config;
      ...
      it('should support regex patterns in include_devices', () => {
        ...
        // Act & Assert
        expect(shouldIncludeDevice(config, 'nous123', 'Nous Device')).to.be
          .true;
